...
class IntersectBasedAnalysisClass:
    ...
    def can_read_list_from_file(self, sector):
        filename = os.path.join(self.data_folder, sector + ".dat")
    ...
    def analyze_sector(self, index):
        ...
        if SECTORS_SET == iSHARES_SECTORS:
            f = open(os.path.join(self.data_folder, 'SectorHoldings.dat'), 'r')
        elif SECTORS_SET == nINESECTORS:
            f = open(os.path.join(self.data_folder, 'FinvizSectors.dat'), 'r')
    ...
    def main(self):
        ...
        if (1):
            ...
            self.out_file = open(os.path.join("logs",filename), "w")
            self.debug_buffers = [[], [], [], [], [], [], [], [], []]
            self.getSpyData()
            self.getSectorsData()
            self.checkIfUpdate()
