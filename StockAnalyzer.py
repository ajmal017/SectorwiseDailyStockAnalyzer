...
class IntersectBasedAnalysisClass:
    ...
    def analyze(self, i_analysisType):
        ...
        for holding in sectorHoldingsUrls:
            ...
            # bm = str(self.stock.generalData['startDate'].month)
            # bd = str(self.stock.generalData['startDate'].day)
            # by = str(self.stock.generalData['startDate'].year)
            # em = str(self.stock.generalData['endDate'].month)
            # ed = str(self.stock.generalData['endDate'].day)
            # ey = str(self.stock.generalData['endDate'].year)
            # freq = 'd'
            # getCmd = 'http://ichart.finance.yahoo.com/table.csv?s=' + \
            #          sector + '&a=' + bm + '&b=' + bd + '&c=' + by + '&d=' + em + '&e=' + ed + '&f=' +\
            #          ey + '&g=' + freq + '&ignore=.csv'
            # response = urllib2.urlopen(getCmd)
            # cr = csv.reader(response) - old way
